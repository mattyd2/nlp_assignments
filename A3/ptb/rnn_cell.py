...
class RNNCell(object):
    ...
    def zero_state(self, batch_size, dtype):
        ...
        if nest.is_sequence(state_size):
            ...
            zeros_flat = [
                array_ops.zeros(
                    array_ops.pack(_state_size_with_prefix(
                        s, prefix=[batch_size])),
                    dtype=dtype)
            ...
            zeros_size = _state_size_with_prefix(
                state_size, prefix=[batch_size])
...
class LSTMCell(RNNCell):
    ...
    def __init__(self, num_units, input_size=None,
        ...
        Args:
          num_units: int, The number of units in the BasicLSTMCell cell
          input_size: Deprecated and unused.
...
class DropoutWrapper(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if (not isinstance(self._input_keep_prob, float) or
            ...
            inputs = nn_ops.dropout(
                inputs, self._input_keep_prob, seed=self._seed)
        output, new_state = self._cell(inputs, state, scope)
        if (not isinstance(self._output_keep_prob, float) or
                self._output_keep_prob < 1):
            output = nn_ops.dropout(
                output, self._output_keep_prob, seed=self._seed)
        ...
        with vs.variable_scope(scope or type(self).__name__):  # "EmbeddingWrapper"
            with ops.device("/cpu:0"):
                if self._initializer:
                    ...
                    # Default initializer for embeddings should have
                    # variance=1.
                ...
                embedding = vs.get_variable(
                    "embedding", [self._embedding_classes,
