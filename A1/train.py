...
tf.flags.DEFINE_float("l2_reg_lambda", 2,
...
# current shape is (10662, 56)
x = np.array(list(vocab_processor.fit_transform(x_text)))

x_train_tmp, x_dev, y_train_tmp, y_dev = train_test_split(
    x, y, test_size=0.10, random_state=42)

x_train, x_val, y_train, y_val = train_test_split(
    x_train_tmp, y_train_tmp, test_size=0.10, random_state=42)

print("Vocabulary Size: {:d}".format(len(vocab_processor.vocabulary_)))
print(
    "Train/Dev split: {:d}/{:d}/{:d}".format(len(y_train), len(y_val), len(y_dev)))
